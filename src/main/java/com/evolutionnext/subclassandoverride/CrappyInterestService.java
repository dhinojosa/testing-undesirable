...
public class CrappyInterestService implements InterestService {
    ...
    protected Float lookupLatestInterestRate() throws IOException {
        ...
        try (Socket socket = new Socket(InetAddress.getByName(host), 80);
            ...
            String line;
            while ((line = bufferedReader.readLine()) != null) return Float.parseFloat(line);
    ...
    @Override
    public synchronized void addToAccount(int months, int amount) throws IOException {
        this.balance += (this.lookupLatestInterestRate() * months * amount) + amount;
    }
